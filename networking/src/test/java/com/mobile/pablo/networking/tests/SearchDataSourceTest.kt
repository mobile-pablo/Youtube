...
class SearchDataSourceTest {
    ...
    @Test
    fun `getSearchVideos returns mapped data when response is successful`() = runBlocking {
        val query = "test"
    ...
    @Test
    fun `getSearchVideos returns network exception when response is not successful`() = runBlocking {
        val query = "dog"
        val notFound = "Not found"

        val error = Response.error<SearchResponse>(
